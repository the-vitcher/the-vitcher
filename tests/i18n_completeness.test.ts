...
// The status registry (src/ui/i18n.status.json, regenerated by pretest) marks a
// key the maintainer has not filled for a locale as `pending`. The build
// English-fills those so dev builds stay usable, which means M16 below would
// otherwise flag every English-only contribution as a "leak" - and an
// English-only change is explicitly legal at the PR tier
// (docs/i18n-scaling/translation-workflow.md). Refusing to SHIP a pending row is
// the release tier's job (I18N_RELEASE_TIER=1, tests/i18n_status_registry.test.ts),
// so M16 skips pending rows and keeps its teeth for the case it actually owns: a
// locale that once translated a key and has silently regressed to English.
...
const statusRegistry: {
  keys: Record<string, { locales?: Record<string, { state?: string }> }>;
} = JSON.parse(fs.readFileSync(registryPath, "utf8"));

function isPending(lang: string, key: string): boolean {
  return statusRegistry.keys[`main:${key}`]?.locales?.[lang]?.state === "pending";
}
...
describe("i18n whole-catalog completeness", () => {
  ...
  it("non-Latin locales ship no untranslated English (only brand/URL leaves stay identical)", () => {
    ...
    for (const lang of nonLatin) {
      const flat = flatten(TABLES[lang]);
      for (const [key, enValue] of Object.entries(enFlat)) {
        if (wordy(enValue) && flat[key] === enValue && !BRAND_ALLOW.has(key) && !isPending(lang, key)) {
