...
import { MOBA_MAX_ABILITY_RANK, MOBA_RANK_POWER, mobaScaleEffectForRank } from '../src/sim/moba';
...
describe('The Clash skill tooltips (rank-resolved)', () => {
  it('previews rank 1 numbers when the ability is not learned yet', () => {
    const html = mobaSkillTooltipHtml(PAN, 0, 'rage');
  ...
  it('shows the current rank standing and a next-rank preview', () => {
    const html = mobaSkillTooltipHtml(PAN, 1, 'rage');
  ...
  it('drops the next-rank line at max rank', () => {
    const html = mobaSkillTooltipHtml(PAN, MOBA_MAX_ABILITY_RANK, 'rage');
    expect(html).toContain(`Rank ${MOBA_MAX_ABILITY_RANK}/${MOBA_MAX_ABILITY_RANK}`);
    expect(html).not.toContain('Next rank');
  });

  it('gives the 4th-slot ultimate the same four-rank ladder as the basics', () => {
    const ultId = MOBA_HEROES.snacko.abilities[MOBA_HEROES.snacko.abilities.length - 1];
    const html = mobaSkillTooltipHtml(ultId, 1, 'rage');
    expect(html).toContain(`Rank 1/${MOBA_MAX_ABILITY_RANK}`);
    const next = abilityEffectText(MOBA_ABILITIES[ultId].effects.map((e) => mobaScaleEffectForRank(e, 2)));
    if (next) expect(html).toContain('Next rank');
  });
  ...
  it('carries the cost, cooldown, and range lines with resolved numbers', () => {
    ...
    const html = mobaSkillTooltipHtml('moba_yoink', 1, 'rage');
  ...
  it('renders every hero kit without leaving an unresolved $d anywhere', () => {
    for (const hero of Object.values(MOBA_HEROES)) {
      for (const id of hero.abilities) {
        for (const rank of [0, 1, MOBA_MAX_ABILITY_RANK]) {
          const html = mobaSkillTooltipHtml(id, rank, 'mana');
  ...
  it('is a safe no-op for unknown ability ids', () => {
    expect(mobaSkillTooltipHtml('nope', 1, 'mana')).toBe('');
  });
});
